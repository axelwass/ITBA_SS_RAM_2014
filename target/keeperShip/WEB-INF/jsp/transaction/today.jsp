...
<h2>Transacciones del d&iacute;a</h2> <h4>${todayDate} </h4><c:if test="${resultPage.pages > 1}"><a href="todayPrintable"> Ver en una p&aacute;gina</a></c:if>
...
<c:if test="${resultPage.pages > 1}">
	<c:forEach var="page" begin="0" end="${resultPage.pages -1}" step="1">
		...
		<a
			href="today?actualPage=${page}&pageSize=${pageSize}"><c:if
