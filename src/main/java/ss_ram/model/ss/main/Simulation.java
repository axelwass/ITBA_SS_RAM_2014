...
import ss_ram.model.ss.memory.MemoryManager;
import ss_ram.model.ss.memory.OperationInfo;
import ss_ram.model.ss.memory.OrderConfiguration;
import ss_ram.model.ss.randoms.RandomGenerator;
		...
		while(running.get() && time<simulationTime){
			...
			if((time % (1000))==0){
				...
				if( image!=null && !image.isUpdated()){
					HistogramInformation histogram = manager.histogram(16, memorySize,time);
