...
<h2>Caja y cheques del d&iacute;a</h2><h4>${todayDate} </h4> <c:if test="${resultPage.pages > 1}"><a href="todayCashNCheckPrintable"> Ver en una p&aacute;gina</a></c:if>
...
<c:if test="${resultPage.pages > 1}">
	<c:forEach var="page" begin="0" end="${resultPage.pages -1}" step="1">
		...
		<a
			href="todayCashNCheck?actualPage=${page}&pageSize=${pageSize}"><c:if
