...
<form:form class="form-horizontal" method="GET" action="searchResults"
	...
	<fieldset>
		...
		<div class="control-group">
			<label class="control-label" for="bank">Ingresado por:</label>
			<div class="controls">
		...
		<div class="control-group">
			<label class="control-label" for="pageSize">Cheques por p&aacute;gina:</label>
			<div class="controls">
...
<c:if test="${resultPage.pages > 0}">
	<c:forEach var="page" begin="0" end="${resultPage.pages -1}" step="1">
	 ...
	 <a href="searchResults?page=${page}&id=${checkSearchForm.id}&number=${checkSearchForm.number}&fromValue=${checkSearchForm.fromValue}&toValue=${checkSearchForm.toValue}&fromPayDate=${checkSearchForm.fromPayDate}&toPayDate=${checkSearchForm.toPayDate}&fromEntryDate=${checkSearchForm.fromEntryDate}&toEntryDate=${checkSearchForm.toEntryDate}&entryBy=${checkSearchForm.entryBy.id}&bank=${checkSearchForm.bank.id}&client=${checkSearchForm.client.id}&pageSize=${checkSearchForm.pageSize}"><c:if test="${page == checkSearchForm.page}"><b></c:if>${page+1}<c:if test="${checkSearchForm.page == page}"></b></c:if></a>
