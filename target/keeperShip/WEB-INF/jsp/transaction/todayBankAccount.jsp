...
<h2>Movimientos del d&iacute;a</h2><h4>${todayDate} </h4> <c:if test="${resultPage.pages > 1}"><a href="todayBankAccountPrintable?bankAccountId=${bankAccount.id}"> Ver en una p&aacute;gina</a></c:if>
...
<c:if test="${resultPage.pages > 1}">
	<c:forEach var="page" begin="0" end="${resultPage.pages -1}" step="1">
		...
		<a
			href="todayBankAccount?bankAccountId=${bankAccount.id}&actualPage=${page}&pageSize=${pageSize}"><c:if
...
<div class="form-actions dont-print">
		<a class="btn btn-primary" href="../bankaccount/addTransaction?baId=${bankAccount.id}">Ingreso</a>
		<a class="btn btn-primary" href="../bankaccount/addNegativeTransaction?baId=${bankAccount.id}">Egreso</a>
</div>
<c:if test="${resultPage.pages <= 1}">
<div class="form-actions dont-print">
